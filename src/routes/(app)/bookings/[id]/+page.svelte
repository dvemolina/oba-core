...
<div class="mx-auto max-w-lg p-4 md:p-6">
	...
	<div class="mb-5 flex items-start gap-3">
		...
		<span
			class="rounded-full px-2.5 py-1 text-xs font-medium {statusColors[data.booking.status]}"
		>
			{data.booking.status}
		</span>
	</div>
			...
			{#each data.booking.clients as bc}
				<div class="space-y-2">
					...
					>
						...
						<div class="pt-4">
							<span
								class="rounded-full px-2 py-0.5 text-xs font-medium {paymentColors[bc.paymentStatus]}"
							>
								{bc.paymentStatus}
							</span>
						</div>
						<button
							type="submit"
							class="mt-4 rounded-lg bg-ocean/10 px-3 py-2 text-xs font-medium text-ocean hover:bg-ocean/20"
						>
							Save
						</button>
					</form>
	...
	<div class="flex gap-3">
		{#if data.booking.status !== 'confirmed'}
			<form method="post" action="?/update" use:enhance class="flex-1">
				...
				<button
					type="submit"
					class="w-full rounded-lg bg-confirmed py-2.5 text-sm font-semibold text-white hover:opacity-90"
				>
					Confirm
				</button>
			</form>
